...
public class ImageData {
	...
	// if invalid indices are given error is printed to sys.err (for debugging
	// purposes
	...
	// transposes the image rotating it 90 degrees, running this twice returns the
	// original image.
	void TransPose() {
	...
	// remove one column from the left hand side of the picture plus any other rows
	// designated for trimming
	void trimLeft() {
		this.trimLeft += 1;
		trim();
	}

	// remove one row from the top side of the picture plus any rows designated for
	// trimming
	void trimTop() {
		this.trimTop += 1;
		trim();
	...
	// remove one column from the right hand side of the picture plus any other rows
	// designated for trimming
	void trimRight() {
		this.trimRight += 1;
		trim();
	...
	// remove one row from the bottom side of the picture plus any rows designated
	// for trimming
	void trimBottom() {
		this.trimBottom += 1;
		trim();
	...
	// before running trim you can set how many rows to remove when you next call
	// it.
	...
	// before running trim you can set how many columns to remove when you next call
	// it.
	// if there is already trim values set these will add to it
	...
	void trim() {
		this.img = img.getSubimage(trimLeft, trimTop, img.getWidth() - trimLeft - trimRight,
				img.getHeight() - trimTop - trimBottom);
		this.trimLeft = 0;
		this.trimRight = 0;
		this.trimTop = 0;
		this.trimBottom = 0;
